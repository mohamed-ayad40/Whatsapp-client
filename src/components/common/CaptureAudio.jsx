...
function CaptureAudio({hide}) {
  ...
  const handleStartRecording = () => {
    ...
    navigator.mediaDevices.getUserMedia({audio: true}).then((stream) => {
      ...
      mediaRecorder.onstop =() => {
        const blob = new Blob(chunks, {type: "audio/oggs; codecs=opus"});
  ...
  const sendRecording = async () => {
    try {
      // Stop the recording if it's still ongoing
      if (isRecording) {
        handleStopRecording();
        // Wait a brief moment for recording to finalize
        await new Promise((resolve) => setTimeout(resolve, 500));
      }
  
      if (!renderedAudio) {
        console.log("No audio recorded or audio is still processing.");
        return;
      }
      ...
      <div className="mx-4 py-2 px-4 text-white text-lg flex gap-3 justify-center items-center bg-search-input-container-background rounded-full drop-shadow-lg">
        ...
        (<div>
          {recordedAudio && (
            <>
              {!isPlaying ? 
                <FaPlay onClick={handlePlayRecording} /> : <FaStop onClick={handlePauseRecording} />  
              }
            </>
          )}
        </div>
